...
export interface BayesianShrinkageOptions {
  /** 先验强度参数，默认为10 */
  priorStrength: number;
}

/** 创建贝叶斯收缩算法引擎 */
export function createBayesianShrinkageEngine(options?: Partial<BayesianShrinkageOptions>): ScoringEngine {
  ...
  return {
    ...
    compute(items: Item[], dimensions: Dimension[]): ScoredItem[] {
      ...
      // 全局加权均值
      const globalWeightedAverage = normalized.reduce(
        (sum, { dimension, normalizedWeight }) =>
          sum + dimAverages[dimension.id] * normalizedWeight,
        0,
      );
      ...
      const scored: ScoredItem[] = items.map(item => {
        ...
        // 加权原始分数
        const rawWeightedScore = normalized.reduce(
          (sum, { dimension, normalizedWeight }) =>
            sum + (item.scores[dimension.id] ?? 0) * normalizedWeight,
          0,
        );
