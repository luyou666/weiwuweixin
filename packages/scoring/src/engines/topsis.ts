...
export const topsisEngine: ScoringEngine = {
  id: 'topsis',
  name: 'TOPSIS理想解',
  ...
  compute(items: Item[], dimensions: Dimension[]): ScoredItem[] {
    ...
    const matrix: number[][] = items.map(item =>
      dimIds.map(dimId => item.scores[dimId] ?? 0),
    );
    ...
    const normalizedMatrix: number[][] = matrix.map(row =>
      row.map((val, colIdx) => colNorms[colIdx] === 0 ? 0 : val / colNorms[colIdx]),
    );

    // 2. 加权归一化矩阵
    const weightedMatrix: number[][] = normalizedMatrix.map(row =>
      row.map((val, colIdx) => val * normalized[colIdx].normalizedWeight),
    );
